...
// Handle status update for Approve/Decline actions
if (isset($_GET['action']) && isset($_GET['id'])) {
    $id = $_GET['id'];
    $action = $_GET['action'];
    
    $validActions = ['Approved', 'Rejected', 'Reviewed'];
    if (in_array($action, $validActions)) {
        $updateSql = "UPDATE ipcr_submissions SET status = ? WHERE id = ?";
        $stmt = $conn->prepare($updateSql);
        $stmt->bind_param("si", $action, $id);
        
        if ($stmt->execute()) {
            $actionText = ($action === 'Rejected') ? 'Declined' : $action;
            $_SESSION['success'] = "Submission has been " . strtolower($actionText) . " successfully!";
            header("Location: view_submission.php?id=$id");
            exit();
        } else {
            $_SESSION['error'] = "Failed to update submission status: " . $conn->error;
        }
    }
}
...
<head>
  ...
  <style>
    ...
    @media print {
      ...
      .topbar, .sidebar, .action-buttons, .dark-toggle, .settings, .notification-bell, .add-row-btn {
...
<div class="sidebar" id="sidebar">
  ...
  <a href="department_reports.php"><i class="fas fa-layer-group me-2"></i> Department </a>
    ...
    <div class="signature-block mt-4">
      ...
      <div class="mt-3">
        ...
        <select name="status" class="form-control w-auto">
          ...
          <option value="Rejected" <?= $data['status'] == 'Rejected' || $data['status'] == 'Declined' ? 'selected' : '' ?>>Declined</option>
        </select>
      </div>
  ...
  <?php else: ?>
    <div class="action-buttons">
      <?php if ($data['status'] === 'Pending'): ?>
        <a href="?id=<?= $id ?>&action=Approved" class="btn btn-success">
          <i class="fas fa-check-circle me-1"></i> Approve
      <?php elseif ($data['status'] === 'Reviewed'): ?>
        <a href="?id=<?= $id ?>&action=Approved" class="btn btn-success">
          <i class="fas fa-check-circle me-1"></i> Approve
        </a>
      <?php endif; ?>
