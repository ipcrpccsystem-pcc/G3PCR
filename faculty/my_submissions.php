...
// Get faculty ID from session with validation
 $facultyId = isset($_SESSION['user_id']) ? (int)$_SESSION['user_id'] : 0;
if ($facultyId <= 0) {
    error_log("Invalid faculty ID in session: " . print_r($_SESSION, true));
...
// Get faculty details for name display with error handling
 $facultyQuery = $conn->prepare("SELECT full_name FROM users WHERE id = ?");
if (!$facultyQuery) {
    error_log("Faculty query preparation failed: " . $conn->error);
    $facultyName = "Unknown Faculty (Query Error)";
} else {
    $facultyQuery->bind_param("i", $facultyId);
    if (!$facultyQuery->execute()) {
        error_log("Faculty query execution failed: " . $facultyQuery->error);
        $facultyName = "Unknown Faculty (Execution Error)";
    } else {
        $facultyResult = $facultyQuery->get_result();
        if ($facultyResult->num_rows === 0) {
            error_log("No faculty found with ID: $facultyId");
            $facultyName = "Unknown Faculty (Not Found)";
        } else {
            $facultyData = $facultyResult->fetch_assoc();
            $facultyName = $facultyData['full_name'] ?? 'Unknown Faculty';
            error_log("Retrieved faculty name for ID $facultyId: " . $facultyName);
        }
    }
    $facultyQuery->close();
}
...
if ($stmt) {
    ...
    if ($stmt->execute()) {
        ...
        while ($row = $result->fetch_assoc()) {
            // Fix Unknown Faculty in each submission
            if (empty($row['faculty_name']) || $row['faculty_name'] === 'Unknown Faculty') {
                $row['faculty_name'] = $facultyName;
            }
            $submissions[] = $row;
        }
    } else {
        error_log("Submissions query execution failed: " . $stmt->error);
        $submissions = [];
    }
    $stmt->close();
} else {
    error_log("Submissions query preparation failed: " . $conn->error);
    $submissions = [];
}
...
// Handle profile data
 $profilePicture = isset($_SESSION['profile_picture']) && $_SESSION['profile_picture'] !== ''
   ...
   : '../uploads/1.png';
 $fullName = $_SESSION['full_name'] ?? $facultyName; // Use facultyName as fallback
...
<head>
    <meta charset="UTF-8">
    <title>My Submissions - IPCR</title>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($submissions as $index => $submission): ?>
                                <tr>
                                    <td><?= $index + 1 ?></td>
                                    <!-- FIXED: Use the faculty_name from the submission, not the session -->
