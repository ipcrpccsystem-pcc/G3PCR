...
if (empty($submission)) {
    ...
    $_SESSION['error'] = "No submission found. Please create a submission first.";
...
// Sanitize the filename to remove special characters that might cause issues
 $safe_faculty_name = preg_replace('/[^A-Za-z0-9]/', '_', $faculty['full_name'] ?? 'Faculty');
 $filename = 'IPCR_' . $safe_faculty_name . '_' . $submissionId . '.pdf';
 $filepath = $pdf_dir . $filename;

// Save the PDF file using the absolute path
 $pdf->Output($filepath, 'F');

// Update the database with the PDF file path
 $update_sql = "UPDATE ipcr_submissions SET pdf_file = ? WHERE id = ?";
 $stmt = $conn->prepare($update_sql);
 $stmt->bind_param("si", $filename, $submissionId);
 $stmt->execute();

// Set success message
 $_SESSION['success'] = "PDF saved successfully!";

// Redirect back to the view submission page
