...
// Get approved submissions for this faculty member
 $stmt = $conn->prepare("SELECT * FROM ipcr_submissions WHERE faculty_id = ? AND status = 'Approved' ORDER BY created_at DESC");
...
<head>
    <meta charset="UTF-8">
    <title>My Approved IPCRs - IPCR</title>
    ...
    <div class="card">
        <div class="card-header bg-white py-3">
            <div class="row align-items-center">
                <div class="col">
                    <h5 class="mb-0">My Approved IPCR Submissions</h5>
                </div>
                <div class="col-auto">
                    <span class="badge bg-success"><?= count($submissions) ?> Approved Submissions</span>
                </div>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($submissions as $index => $submission): ?>
                                <tr>
                                    <td><?= $index + 1 ?></td>
                                    <td><?= htmlspecialchars($submission['faculty_name'] ?? $fullName) ?></td>
            ...
            <?php else: ?>
                <div class="text-center py-5">
                    ...
                    <h5>No approved submissions found</h5>
                    <p class="text-muted">You don't have any approved IPCR submissions yet.</p>
                    <a href="Submit_IPCR.php" class="btn btn-primary">
                        <i class="fas fa-plus me-2"></i> Submit New IPCR
                    </a>
                </div>
            <?php endif; ?>
        </div>
